...
class KrogerOSDBackupsAccelerator(Accelerator):
    display_name = 'KROG - OSD Backups Downloader'
    group_name = 'Kroger'
    ...
    async def run(self, invoice_numbers: list[str]) -> str:
        ...
        if '/vendorsso/dashboard' in self.driver.current_url:
            ...
            await self.info('levante page is Loaded  please wait.')
            wait.until(lambda driver: self.driver.execute_script('return document.readyState') == 'complete')
            try:
                ...
                if len(str(invoice_number)) > 9:
                    ...
                    for filename in os.listdir(self.output_path):
                        if filename.startswith('AssociatedDeductions_Invoice'):
                            os.rename(os.path.join(self.output_path, filename), os.path.join(self.output_path, f'KROG-OSD-Backups_Downloader_InvoiceNUM{invoice_number}_file_{filename}'))
                    await self.info(f"this Kroger_Backups{invoice_number} data is being Processed")
            count_invoices = len(invoice_numbers)
            current_date = datetime.today().strftime('%Y%m%d')
            zip_filename = os.path.join(self.output_path, f"KROG - OSD Backups Downloader_{count_invoices}_{current_date}")
